...
namespace sjtu
{
  ...
  public:
    ...
    public:
      ...
      {
        if (!nodePointer || nodePointer == mapPointer->tail)
      ...
      iterator operator--(int)
      {
        if (!nodePointer || nodePointer->pre == mapPointer->head)
          throw invalid_iterator();
        iterator it(*this);
    ...
    linked_hashmap(const linked_hashmap &other)
    {
      ...
      tail->pre = head;
      memset(storageList, 0, Capacity * sizeof(Node *));
      for (Node *i = other.head->nxt; i != other.tail; i = i->nxt)
        insert(*(i->value));
    }
    ...
    T &at(const Key &key)
    {
      ...
      if (it != end())
        return it.nodePointer->value->second;
    ...
    std::pair<iterator, bool> insert(const value_type &value)
    {
      ...
      NodeNum++;
      if (NodeNum > Capacity * LoadFactor)
        reSize();
    ...
    {
      if (pos.mapPointer != this || pos.nodePointer == nullptr || pos == this->end())
        throw index_out_of_bound();
