...
class map {
 ...
 private:
  ...
  AvlNode *privatefind(const Key &x, AvlNode *root_) const {
    if (root_ == nullptr || (!Compare()(root_->data->first, x) && !Compare()(x, root_->data->first)))return root_;

    if (Compare()(x, root_->data->first))return privatefind(x, root_->left);
    else return privatefind(x, root_->right);
  }
  ...
  AvlNode *findNext(AvlNode *node_) const {
    ...
    if (node->right) {
      ...
      while (node->father) {
        if (Compare()(oldNode->data->first, node->father->data->first))return node->father;
        else {
          node = node->father;
        }
      }
  ...
  AvlNode *findPre(AvlNode *node) const {
    if (node == tail)return findMax(root);
    if (node->left) {
      node = node->left;
      ...
      AvlNode *oldNode = node;
      while (node->father) {
        if (Compare()(node->father->data->first, oldNode->data->first))return node->father;
        else {
          node = node->father;
        }
      }
  ...
  void CheckRtAndConnect(AvlNode *t, AvlNode *tmp) {
    if (t->father == nullptr) root = tmp;
    else if (t->father->left == t)t->father->left = tmp;
    else if (t->father->right == t)t->father->right = tmp;
  }
  ...
  void insert(const value_type &x, AvlNode *&t, AvlNode *father_) {
    if (t == nullptr) {
      ...
      if (!father_)root = t;
        ...
        if (Compare()(x.first, father_->data->first))father_->left = t;
        else father_->right = t;
      }
  ...
  void remove(AvlNode *t) {
    ...
    if (t->left && t->right) {
      ...
      if (tmp == t->right) {
        ...
        tmp->right = oldNode;
        oldNode->father = tmp;
        if (oldNode->right)oldNode->right->father = oldNode;
        tmp->father = t->father;
        tmp->height = t->height;
      } else {
        tmp->left = t->left;
  ...
  void LL(AvlNode *t) {
    ...
    if (newroot->right)newroot->right->father = t;
  ...
  void RR(AvlNode *t) {
    ...
    if (newroot->left)newroot->left->father = t;
    newroot->father = t->father;
 ...
 public:
  ...
  class iterator {
   public:
    ...
    iterator operator++(int) {
      if (node == target->tail) throw invalid_iterator();
      iterator it = *this;
    ...
    iterator operator--(int) {
      iterator it = *this;
      if (*this == target->begin())throw invalid_iterator();
  ...
  class const_iterator {
   ...
   public:
    ...
    const_iterator operator--(int) {
      const_iterator it = *this;
      if (it == target->cbegin())throw invalid_iterator();
      if (!node)throw invalid_iterator();
      this->node = target->findPre(node);
      return it;
    }
  ...
  T &at(const Key &key) {
    ...
    if (carrier)return carrier->data->second;
  ...
  iterator begin() {
    if (root)return iterator(findMin(root), this);
    return iterator(tail, this);
  }
  const_iterator cbegin() const {
    AvlNode *carrier = findMin(root);
    if (root)return const_iterator(carrier, this);
 ...
 */
  void erase(iterator pos) {
    if (pos.node == nullptr || pos.target != this || pos.node == tail)throw index_out_of_bound();
    num--;
    remove(pos.node);
  }
/**
 ...
 */
  iterator find(const Key &key) {
    AvlNode *carrier = privatefind(key, root);
    if (carrier)return iterator(carrier, this);
    return iterator(tail, this);
  }
  const_iterator find(const Key &key) const {
    AvlNode *carrier = privatefind(key, root);
    if (carrier)return const_iterator(carrier, this);
    return const_iterator(tail, this);
  }
};
