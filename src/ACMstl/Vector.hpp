...
//    using iterator_category = std::output_iterator_tag;
   ...
   public:
    ...
    int operator-(const iterator &rhs) const {
      ...
      if (vector_head != rhs.vector_head)throw invalid_iterator();//NOLINT
...
//    token_size = 0;
  ...
  vector &operator=(const vector &other) {
    ...
    for (int i = 0; i < real_size; i++)delete storage[i];
    delete[]storage;
    storage = new T *[other.token_size];
    for (int i = 0; i < other.real_size; i++) {
      storage[i] = new T(*other.storage[i]);
  ...
  T &at(const size_t &pos) {
    if (pos < 0 || pos >= real_size)throw index_out_of_bound();//NOLINT
    return **(storage + pos);
  }
   ...
   */
  const T &front() const {
    if (!real_size) throw container_is_empty();//NOLINT
    return **storage;
  }
  /**
...
//      storage[i] = new T(*carrier[i]);
  ...
  iterator insert(iterator pos, const T &value) {
    ...
    if ((real_size + 1) >= token_size) doublespace();
//    if (real_size > 0) {
...
//    }
  ...
  iterator insert(const size_t &ind, const T &value) {
    if (ind > real_size)throw index_out_of_bound();//NOLINT
    if ((real_size + 1) >= token_size) doublespace();
//    if (real_size > 0) {
//      storage[real_size] = new T(*storage[real_size - 1]);
...
//    storage[real_size] = nullptr;
   ...
   */
  void pop_back() {
    if (real_size == 0)throw container_is_empty();//NOLINT
