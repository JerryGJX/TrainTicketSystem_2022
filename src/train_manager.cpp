...
TrainManager::TrainManager(const std::string &filename,
  ...
  if (basicTrainBackUp.size()) {
    ...
    for (auto &T: ca)basicTrainDatabase.insert(T);
...
void TrainManager::deleteTrain(const std::string &trainID_) {
  ...
  for (int i = 0; i < b_tr_ca.stationNum; ++i) {
    stationDataBase.erase(std::make_pair(std::make_pair(CalHash(tr_ca.stations[i]), b_tr_ca.startSellDate), tr_hash));
  }
}
...
void TrainManager::releaseTrain(const std::string &trainID_) {
  ...
  for (int i = 0; i < b_tr_ca.stationNum; ++i) {
    ...
    stationDataBase.insert(std::make_pair(std::make_pair(std::make_pair(CalHash(tr_ca.stations[i]),
                                                                        b_tr_ca.startSellDate), CalHash(trainID_)),
                                          train_station));
  }
...
bool TrainManager::queryTrain(const std::string &trainID_,
  ...
  bool if_added = true;
  DayTrain dt_ca;
  if (!DayTrainToSeat.find(std::make_pair(wanted_date, tr_hash), dt_ca))if_added = false;
  ...
  for (int i = 1; i < b_tr_ca.stationNum - 1; ++i) {
    ...
    st_ca += std::string(tr_ca.stations[i]) + " " +
        ...
        + " " + std::to_string(tr_ca.sumPrice[i]) + " ";
...
void TrainManager::QueryTicket(sjtu::linked_hashmap<std::string, std::string> &info,
  ...
  stationDataBase.range_search(std::make_pair(std::make_pair(start_hash, 0), 0),
                               std::make_pair(std::make_pair(start_hash, wanted_date), 0),
                               result_start);
  stationDataBase.range_search(std::make_pair(std::make_pair(terminal_hash, 0), 0),
                               std::make_pair(std::make_pair(terminal_hash, wanted_date), 0),
                               result_terminal);
...
//  }
  ...
  for (int i = 0; i < possible_ans.size(); i++) {
    ...
    bool if_added = true;
    DayTrain dt_ca;
    if (!DayTrainToSeat.find(std::make_pair(start_date, tr_hash), dt_ca))if_added = false;
...
//  stationDataBase.range_search(std::make_pair(terminal_hash, 0), std::make_pair(terminal_hash + 1, 0), result_terminal);
  ...
  for (auto it_s = startTime_permit.begin(); it_s != startTime_permit.end(); it_s++) {
    ...
    DayTrain tr_1;
    if (!DayTrainToSeat.find(std::make_pair(start_date_1, f_tr_hash), tr_1))if_added_tr1 = false;
    ...
    for (auto &T: result_terminal) {
      ...
      for (int i = 0; i < T.rank; ++i) {
        ...
        DayTrain tr_2;
        if (!DayTrainToSeat.find(std::make_pair(best_trans_day, t_tr_hash), tr_2))if_added_tr2 = false;
...
//  trainDataBase.find(tidHash, wanted_train);
  ...
  if (!stationDataBase.find(std::make_pair(std::make_pair(start_hash,b_tr_ca.startSellDate), tr_hash), f_ts_ca) ||
      !stationDataBase.find(std::make_pair(std::make_pair(terminal_hash,b_tr_ca.startSellDate), tr_hash), t_ts_ca))
    return "-1";

  int levT_f = f_ts_ca.leavingTime, arvT_t = t_ts_ca.arrivingTime, f_rank = f_ts_ca.rank, t_rank = t_ts_ca.rank;
  if (f_rank > t_rank)return "-1";
