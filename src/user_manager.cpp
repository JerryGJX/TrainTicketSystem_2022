...
bool UserManager::AddUser(std::string *info)
{
  if (userDatabase.size())
  {
    ...
    if (onlineUser[CalHash(info[JerryGJX::_c])] <= std::stoi(info[JerryGJX::_g]))
    ...
    info[JerryGJX::_g] = std::to_string(10);
                                ...
                                std::string &result)
{
  ...
  if (!info[JerryGJX::_p].empty())
    ta_ca.password = info[JerryGJX::_p];
  if (!info[JerryGJX::_n].empty())
    ta_ca.name = info[JerryGJX::_n];
  if (!info[JerryGJX::_m].empty())
    ta_ca.mailAddr = info[JerryGJX::_m];
  if (!info[JerryGJX::_g].empty())
  {
    ...
    //      return false;
    if (isLogin(info[JerryGJX::_u]))
      onlineUser[CalHash(info[JerryGJX::_u])] = ta_ca.privilege;
  }
