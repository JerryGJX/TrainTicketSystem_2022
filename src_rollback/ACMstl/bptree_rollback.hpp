...
//BpTree正文
 public:
  ...
  bool insert(const Key &key, const Value &value, int time) {
    ...
    if (flag) {
      ++_size;
      if (!is_rollback) {
        operation_info info(time, _insert_, key, value);
        stack_for_rollback.push(info);
      }
    }
    return flag;
  }
  //另一个版本的insert
  bool insert(const std::pair<Key, Value> &data, int time) {
    return insert(data.first, data.second, time);
  }
  //删除节点，失败返回0
  bool erase(const Key &key, int time) {
    Value value;
    Bptree_normal_node virtual_root;
    bool flag = dfs_erase(root, 0, 0, virtual_root, key, value);
    if (flag) {
      --_size;
      if (!is_rollback) {
        operation_info info(time, _erase_, key, value);
        stack_for_rollback.push(info);
      }
    }
  ...
  bool modify(const Key &key, const Value &value, int time) {
    //Cache
    CacheModify(key, value);

    Value value2;
    int flag = dfs_modify(root, 0, key, value, value2);
    if (flag != -1 && !is_rollback) {
      operation_info info(time, _modify_, key, value2);
      stack_for_rollback.push(info);
    }
    return flag != -1;
  }
  //另一个版本的modify
  bool modify(const std::pair<Key, Value> &data, int time) {
    return modify(data.first, data.second, time);
  }
  ...
  void rollback(int endtime) {
    if (stack_for_rollback.empty()) return;
    is_rollback = 1;
    operation_info info;
    stack_for_rollback.top(info);
    CacheClear();
    while (info.time > endtime) {
      switch (info.type) {
        case _insert_ :erase(info.key, 0);
          break;
        case _erase_ :insert(info.key, info.value, 0);
          break;
        case _modify_ :modify(info.key, info.value, 0);
          break;
      }
      stack_for_rollback.pop();
      if (!stack_for_rollback.empty()) {
        stack_for_rollback.top(info);
      } else {
        info.time = -1;
      }
 ...
 private:
  ...
  int dfs_modify(int pos, bool is_leaf, const Key &key, const Value &value, Value &value2) {
    if (is_leaf) {
      Bptree_leaf_node node;
      leaf_node_manager.read(node, pos);
      for (int i = 0; i < node.size && node.key_list[i] <= key; ++i) {
        if (node.key_list[i] == key) {
          value2 = node.value_list[i];
  ...
  //erase的辅助函数
  bool dfs_erase(int pos, bool is_leaf, int rank, Bptree_normal_node &parent, const Key &key, Value &value2) {
    if (is_leaf) {
      Bptree_leaf_node self;
      leaf_node_manager.read(self, pos);
      bool flag = 0;
